...
| [invoke](#invoke) | get the id of an entry (citation key). |
| [cite](#cite) | get the id of an entry (citation key), filtering on its content. |
...
| `-r` | search a pattern in reading notes | [regex](https://www.postgresql.org/docs/current/functions-matching.html#FUNCTIONS-POSIX-REGEXP) |
| `-s` | search lemmas/lexemes in reading notes using `tsquery` | [tsquery](https://www.postgresql.org/docs/current/datatype-textsearch.html#DATATYPE-TSQUERY) |
...
```bash
# select entries that contains tsquery 'archi:*'.
retrolire -s 'archi:*'
```
...
Command `invoke` is used to get the __ID__ (citation key) of an entry, for example, to be inserted in a [pandoc](https://pandoc.org/)-markdown document for which footnotes and bibliography will be automatically produced with [citeproc](https://github.com/jgm/citeproc).

```bash
retrolire invoke | xclip -selection clipboard
```

### cite

`cite` produces the same output as `invoke`, but the filtering with FZF is done on entry's content instead of title and authors.
...
- [ncurses](https://packages.debian.org/fr/sid/libncurses)
...
- [x] parse and index paragraphs in notes.
...
- [x] rename actions (cite and invoke)
...
- [x] line number for indexed objects (ideas, quotes, ...) to edit directly.
