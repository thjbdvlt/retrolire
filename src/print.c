...
#define size 100
  ...
  char* x = memccpy(margin + s, "\\  ", '\0', size - (size_t)s);
  if (!x) {
    fputs("error creating margins for printing.\n", stderr);
    return 0;
  }
#undef size
...
#define CMD "bat -p "
  ...
  char fname[sizeof(FILEPATH) + 1] = FILEPATH; // initiate file name
  int fd = mkstemp(fname); // create temporary file
  if (fd == -1) {
    fputs("error creating temporary file.\n", stderr);
    return 0;
  }

  FILE* f = fdopen(fd, "w"); // open file
  if (!f) {
  ...
  write_res_expanded(res, f, term_width); // write result to file
  ...
  // copy file name
  if (!memccpy(x - 1, fname, '\0', csize)) {
    fputs("error (list).\n", stderr);
    remove(fname);
    return 0;
  }

  system(cmd);   // call less command
  remove(fname); // remove temporary file
  return 1;
#undef FILEPATH
#undef cmd
}
...
preview(char* id)
{
  ...
  if (PQresultStatus(res) == PGRES_TUPLES_OK)
    write_res_expanded(res, stdout, get_term_width());
  PQclear(res);
...
list_authors()
{
  return list_anything("select name from authors");
}
...
list_tags()
{
  return list_anything("select distinct tag from tag");
}
