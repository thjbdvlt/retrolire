...
func initDB(t *CliState) {
	var err error
	var db *sql.DB
	// Create polyanthea directory if it doesn't exists.
	// I do this without using `state.NoErr` because log file is in polyanthea directory.
	if !files.FileExists(config.Directory) {
		err := os.Mkdir(config.Directory, 0750)
		if err != nil {
			fmt.Fprintln(os.Stderr, "couldn't create polyanthea directory")
			os.Exit(1)
		}
	}
	t.CD()
