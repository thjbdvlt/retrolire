...
func (s *MainState) conn() *sql.DB {
	...
	if !s.dbChecked {
		...
		if err != nil {
			fmt.Println(
				"Looks like there's no otlet database in this directory: ",
				config.Directory,
				"\nYou should call `otlet init` to create the database.",
			)
			os.Exit(1)
		}
	}
...
// Root - Open otlet directory as root
func (s *MainState) Root() *os.Root {
	var err error
	root, err := os.OpenRoot(config.Directory)
	NoErr(s, err, "couldn't open otlet directory", config.Directory)
	return root
}
